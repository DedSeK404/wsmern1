...
export const addproduct = (input) => async (dispatch) => {
  ...
  try {
    ...
    const postProduct = await axios.post("/product/add", { ...input });
  } catch (error) {
    dispatch({ type: FAILD, payload: error });
  }
};
...
export const editproduct = (editData, idprod) => async (dispatch) => {
  ...
  try {
    ...
    const patchProduct = await axios.patch(`/product/${idprod}`, {
      ...editData,
    });
  } catch (error) {
    dispatch({ type: FAILD, payload: error });
...
export const deleteproduct = (idprod) => async (dispatch) => {
  ...
  try {
    ...
    const deleteProduct = await axios.delete(`/product/${idprod}`);
  } catch (error) {
    dispatch({ type: FAILD, payload: error });
  }
};
